  ...
  async function getFarmerPostData(indices) {
    try {
      ...
      for (let i = 0; i < indices.length; i++) {
        const result = await contract.methods.Farmer_Post_Array(indices[i]).call();
  ...
  const get_Own_Product = async () => {
    try {
      ...
      const distributor = await contract.methods
        .get_Owned_Properties(accounts[0], 0).call();
      ...
      </div>
      {
        ...
        <div className="md:w-[50%] flex flex-col gap-10">
          ...
          <form className="flex flex-col md:gap-6 gap-4">
            <input type="file" name="" id="" placeholder="Image" className="w-full border rounded-xl  py-4 px-4 outline-none" onChange={(e) => setImg(e.target.files[0])}/>
            <input type="text" name="" id="" placeholder="Name" className="w-full border rounded-xl  py-4 px-4 outline-none" onChange={(e) => setName(e.target.value)}/>
            <textarea name="" id="" cols="30" rows="10" placeholder="Description" className="resize-none w-full border rounded-xl  py-4 px-4 outline-none" onChange={(e) => setDescription(e.target.value)}></textarea>
            <input type="text" name="" id="" placeholder="Quantity" className="w-full border rounded-xl  py-4 px-4 outline-none" onChange={(e) => setQuantity(e.target.value)}/>
            <input type="text" name="" id="" placeholder="Price" className="w-full border rounded-xl  py-4 px-4 outline-none" onChange={(e) => setPrice(e.target.value)}/>
            <button className="w-full border rounded-xl  py-4 px-4 bg-black font-bold text-white" type="submit" onClick={submitAndBlock}>Submit Now</button>
          </form>
        </div>
        ...
        {farmerPosts.length != 0 ? farmerPosts.map((post, index) => (
          <>
            <Card
              state={state}
              id={post.id}
